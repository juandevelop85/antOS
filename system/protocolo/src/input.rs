//! antOS Input Protocol and Event System.
//!
//! Provides typed input event representations (keys, mouse, scroll),
//! modifiers tracking, keyboard layout mapping, and scancode decoders
//! for PS/2 Set 1 and Linux EV_KEY (VirtIO-Input standard).
...
/// Linux Input Subsystem Event Types (as used by VirtIO Input).
...
/// Absolute axes for tablets/touchscreens.
pub const ABS_X: u16 = 0x00;
pub const ABS_Y: u16 = 0x01;

/// Mouse button codes in Linux EV_KEY.
...
mod tests {
    ...
    #[test]
    fn test_linux_ev_key_decoder() {
        assert_eq!(
            decode_linux_ev(EV_KEY, 30, 1),
            Some(InputEvent::KeyPress(KeyCode::KeyA))
        );
        assert_eq!(
            decode_linux_ev(EV_KEY, 30, 0),
            Some(InputEvent::KeyRelease(KeyCode::KeyA))
        );
        assert_eq!(
            decode_linux_ev(EV_KEY, 28, 1),
            Some(InputEvent::KeyPress(KeyCode::Enter))
        );
        assert_eq!(
            decode_linux_ev(EV_KEY, 125, 1),
            Some(InputEvent::KeyPress(KeyCode::LeftSuper))
        );
    }

    #[test]
    fn test_linux_ev_mouse_and_rel_decoder() {
        assert_eq!(
            decode_linux_ev(EV_KEY, BTN_LEFT, 1),
            Some(InputEvent::MouseButtonPress(MouseButton::Left))
        );
        assert_eq!(
            decode_linux_ev(EV_KEY, BTN_LEFT, 0),
            Some(InputEvent::MouseButtonRelease(MouseButton::Left))
        );
        assert_eq!(
            decode_linux_ev(EV_KEY, BTN_RIGHT, 1),
            Some(InputEvent::MouseButtonPress(MouseButton::Right))
        );

        assert_eq!(
            decode_linux_ev(EV_REL, REL_X, 15),
            Some(InputEvent::MouseMove { dx: 15, dy: 0 })
        );
        assert_eq!(
            decode_linux_ev(EV_REL, REL_Y, (-8i32) as u32),
            Some(InputEvent::MouseMove { dx: 0, dy: -8 })
        );
        assert_eq!(
    ...
    #[test]
    fn test_keyboard_state_modifier_tracking() {
        let mut state = KeyboardState::new();
        assert!(!state.modifiers.shift);
        assert!(!state.modifiers.ctrl);

        state.update(&InputEvent::KeyPress(KeyCode::LeftShift));
        assert!(state.modifiers.shift);

        state.update(&InputEvent::KeyPress(KeyCode::LeftCtrl));
        assert!(state.modifiers.ctrl);

        state.update(&InputEvent::KeyRelease(KeyCode::LeftShift));
        assert!(!state.modifiers.shift);
        assert!(state.modifiers.ctrl);

        state.update(&InputEvent::KeyPress(KeyCode::CapsLock));
    ...
    #[test]
    fn test_key_to_ascii_translation() {
        let mut state = KeyboardState::new();

        // Lowercase
        assert_eq!(state.key_to_char(KeyCode::KeyA), Some('a'));
        assert_eq!(state.key_to_char(KeyCode::KeyZ), Some('z'));
        assert_eq!(state.key_to_char(KeyCode::Num1), Some('1'));
        assert_eq!(state.key_to_char(KeyCode::Minus), Some('-'));

        // Shift pressed
        state.update(&InputEvent::KeyPress(KeyCode::LeftShift));
        assert_eq!(state.key_to_char(KeyCode::KeyA), Some('A'));
        assert_eq!(state.key_to_char(KeyCode::KeyZ), Some('Z'));
        assert_eq!(state.key_to_char(KeyCode::Num1), Some('!'));
        assert_eq!(state.key_to_char(KeyCode::Num2), Some('@'));
        assert_eq!(state.key_to_char(KeyCode::Minus), Some('_'));
        assert_eq!(state.key_to_char(KeyCode::Equal), Some('+'));

        // Release shift, toggle caps lock
        state.update(&InputEvent::KeyRelease(KeyCode::LeftShift));
        state.update(&InputEvent::KeyPress(KeyCode::CapsLock));
        assert_eq!(state.key_to_char(KeyCode::KeyA), Some('A'));
        assert_eq!(state.key_to_char(KeyCode::Num1), Some('1')); // CapsLock does not affect numbers
    }

    #[test]
    fn test_roundtrip_serde() {
        let event = InputEvent::KeyPress(KeyCode::KeyK);
