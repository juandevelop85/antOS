...
pub fn cmd_pkg(ctx: &Ctx, args: &[String]) -> Result<()> {
    ...
    match sub {
        "install" | "add" | "i" => {
            let pkg_or_recipe = args.get(1).ok_or_else(|| {
            ...
            let status_badge = if rep.success {
                paint("INSTALADO", GREEN)
            } else {
                paint("ERROR", RED)
            };
            ...
            if !rep.desktop_entries_linked.is_empty() {
                println!(
                    "  Accesos XDG:   {}",
                    paint(&rep.desktop_entries_linked.join(", "), GREEN)
                );
            }
        ...
        "remove" | "rm" | "uninstall" => {
            let pkg = args
                .get(1)
                .ok_or_else(|| anyhow::anyhow!("Uso: antos pkg remove <nombre_paquete>"))?;
            println!(
                "\n{} Desvinculando paquete del perfil activo...",
                paint("antOS antpkg ·", BOLD)
            );
            let rep = crate::pkg::PackageEngine::remove(&ctx.state, pkg)?;
            println!(
                "  {} Paquete «{}» desvinculado.",
                paint("✓", GREEN),
                paint(pkg, BOLD)
            );
            println!(
        ...
        "list" | "ls" => {
            ...
            if gui_only {
                pkgs.retain(|p| {
                    p.app_type == antos_protocol::PackageAppType::Gui || p.desktop_entry.is_some()
                });
            }
        ...
        "apps" | "desktop" | "gui" => {
            println!(
                "\n{} Aplicaciones de Escritorio Registradas:",
                paint("antOS antpkg ·", BOLD)
            );
            let apps = crate::pkg::PackageEngine::list_desktop_apps(&ctx.state)?;
            if apps.is_empty() {
                println!(
                    "  (no hay aplicaciones de escritorio con archivo .desktop registradas)\n"
                );
            } else {
                for app in &apps {
        ...
        "search" | "find" => {
            ...
            println!(
                "\n{} Búsqueda en el catálogo oficial antpkg:",
                paint("antOS antpkg ·", BOLD)
            );
            let results = crate::pkg::PackageEngine::search_catalog(term)?;
            if results.is_empty() {
                println!(
                    "  (no se encontraron recetas coincidentes con «{}»)\n",
                    term
                );
            } else {
                println!(
                    "  Recetas encontradas en el catálogo ({}):\n",
                    results.len()
                );
                for p in &results {
                    let type_badge = match p.app_type {
        ...
        "info" | "show" => {
            let pkg_name = args
                .get(1)
                .ok_or_else(|| anyhow::anyhow!("Uso: antos pkg info <nombre_paquete|receta>"))?;
            ...
            if let Some(p) = found {
                ...
                println!(
                    "  Tamaño:          {:.2} KiB",
                    p.installed_size_bytes as f64 / 1024.0
                );
                ...
                if let Some(d) = &p.desktop_entry {
                    ...
                    if let Some(gn) = &d.generic_name {
                        println!("  Generic Name:    {}", gn);
                    }
                    if let Some(c) = &d.comment {
                        println!("  Comentario:      {}", c);
                    }
                    println!("  Exec:            {}", d.exec);
                    if let Some(ic) = &d.icon {
                        println!("  Icono:           {}", ic);
                    }
                    println!("  Categorías:      {}", d.categories.join(", "));
                    if !d.mime_types.is_empty() {
                        println!("  Tipos MIME:      {}", d.mime_types.join(", "));
                    }
                ...
                match crate::pkg::PackageEngine::resolve_manifest(pkg_name) {
                    Ok(m) => {
                        ...
                        println!(
                            "\n{} Información de Receta Oficial (Catálogo antpkg):",
                            paint("antOS antpkg ·", BOLD)
                        );
                        ...
                        if !m.icons.is_empty() {
                            let icon_summary: Vec<String> = m
                                .icons
                                .iter()
                                .map(|i| format!("{}.{}", i.resolution, i.format))
                                .collect();
                        ...
                        println!(
                            "  Estado:          {}",
                            paint(
                                "Disponible para instalación (no instalado en perfil activo)",
                                YELLOW
                            )
                        );
        ...
        "validate" => {
            let file_or_content = args
                .get(1)
                .ok_or_else(|| anyhow::anyhow!("Uso: antos pkg validate <archivo.desktop>"))?;
            ...
            println!(
                "\n{} Validando archivo .desktop según especificación Freedesktop...",
                paint("antOS antpkg ·", BOLD)
            );
        ...
        "rollback" | "revert" => {
            ...
            println!(
                "\n{} Revirtiendo perfil de paquetes de forma atómica...",
                paint("antOS antpkg ·", BOLD)
            );
            let rep = crate::pkg::PackageEngine::rollback(&ctx.state, target_gen)?;
            println!(
                "  {} Rollback exitoso a la generación {}.",
                paint("✓", GREEN),
                paint(&rep.generation.to_string(), CYAN)
            );
        ...
        _ => {
            ...
            println!(
                ...
                paint(&st.store_path, CYAN)
            );
            println!(
                "  • Directorio de Perfil:    {}",
                paint(&st.current_profile_path, CYAN)
            );
            println!(
                "  • Generación Activa:       {}",
                paint(&st.current_generation.to_string(), BOLD)
            );
