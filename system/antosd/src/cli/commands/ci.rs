    ...
    match sub {
        "status" | "estado" => {
            ...
            println!(
                "\n{}",
                paint(
                    "antOS CI · Reporte del Último Pipeline Ejecutado (T20.3)",
                    BOLD
                )
            );
            if let Some(r) = report {
                ...
                println!(
                    "  Seguridad (Secretos): {}",
                    if r.security_clean {
                        paint("Limpio (0 detectados)", GREEN)
                    } else {
                        paint(
                            &format!("ALERTA: {} detectados", r.secrets_found.len()),
                            RED,
                        )
                    }
                );
        ...
        _ => {
            ...
            println!(
                "\n{}",
                paint(
                    "⚙️ antOS CI · Ejecutando Matriz Local Paralela en Sandboxes (T20.3)",
                    BOLD
                )
            );
            ...
            for s in &report.stages {
                ...
                println!(
                    "    • {:<12} {} ({} ms) — {}",
                    s.name, badge, s.duration_ms, s.command
                );
    ...
    match sub {
        "install" | "instalar" => {
            println!(
                "\n{}",
                paint(
                    "🪝 antOS Git Hooks · Instalando Protección Automatizada (T20.3)",
                    BOLD
                )
            );
        ...
        "uninstall" | "desinstalar" | "remove" => {
            println!(
                "\n{}",
                paint(
                    "🪝 antOS Git Hooks · Desinstalando Protección (T20.3)",
                    BOLD
                )
            );
        ...
        "check" | "audit" => {
            println!(
                "\n{}",
                paint(
                    "🪝 antOS Git Hook Auditor · Verificación de Pre-Commit (T20.3)",
                    BOLD
                )
            );
            ...
            if passed {
                println!(
                    "  {}",
                    paint(
                        "✅ Auditoría superada: sin fugas de secretos detectadas.",
                        GREEN
                    )
                );
                println!();
                ...
                println!(
                    "  {}",
                    paint(
                        "❌ AUDITORÍA RECHAZADA: se detectaron problemas en el código:",
                        RED
                    )
                );
                for e in &errors {
        ...
        _ => {
            ...
            println!(
                "  Pre-commit hook: {}",
                if status.pre_commit_installed {
                    paint("ACTIVO (Vigilando commits)", GREEN)
                } else {
                    paint("INACTIVO", DIM)
                }
            );
            println!(
                "  Pre-push hook:   {}",
                if status.pre_push_installed {
                    paint("ACTIVO (Vigilando push)", GREEN)
                } else {
                    paint("INACTIVO", DIM)
                }
            );
