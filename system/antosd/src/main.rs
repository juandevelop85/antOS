...
fn cmd_ci(ctx: &Ctx, args: &[String]) -> Result<()> {
    let sub = args.first().map(String::as_str).unwrap_or("run");
    match sub {
        "status" | "estado" => {
            let report = ci::CiEngine::get_last_report(&ctx.state)?;
        ...
        _ => {
            ...
            let report = ci::CiEngine::run_pipeline(
                &ctx.workspace,
                &ctx.state,
                stage_filter,
                fast_mode,
...
fn cmd_hook(ctx: &Ctx, args: &[String]) -> Result<()> {
    let sub = args.first().map(String::as_str).unwrap_or("status");
    match sub {
        "install" | "instalar" => {
            println!("\n{}", paint("🪝 antOS Git Hooks · Instalando Protección Automatizada (T20.3)", BOLD));
            let status = ci::CiEngine::install_git_hooks(&ctx.workspace)?;
        ...
        "uninstall" | "desinstalar" | "remove" => {
            ...
            let status = ci::CiEngine::uninstall_git_hooks(&ctx.workspace)?;
        ...
        "check" | "audit" => {
            ...
            let (passed, errors) = ci::CiEngine::run_pre_commit_check(&ctx.workspace)?;
            if passed {
        ...
        _ => {
            let status = ci::CiEngine::query_git_hooks_status(&ctx.workspace)?;
...
fn cmd_snapshot(ctx: &Ctx, args: &[String]) -> Result<()> {
    let sub = args.first().map(String::as_str).unwrap_or("list");
    match sub {
        "create" | "crear" | "save" => {
            let mut label = None;
            ...
            let meta = time_machine::TimeMachineEngine::create_snapshot(
                &ctx.workspace,
                &ctx.state,
                label,
                author,
        ...
        "list" | "ls" | "status" => {
            let list = time_machine::TimeMachineEngine::list_snapshots(&ctx.state)?;
        ...
        "restore" | "restaurar" | "revert" => {
            ...
            let res = time_machine::TimeMachineEngine::restore_snapshot(
                &ctx.workspace,
                &ctx.state,
                id_or_label,
                create_rescue,
        ...
        "delete" | "del" | "rm" | "eliminar" => {
            ...
            let deleted = time_machine::TimeMachineEngine::delete_snapshot(&ctx.state, id_or_label)?;
