...
- **Tormenta de IRQ en `-M virt` con periféricos PCI:** el arranque AArch64 se colgaba tras `GICv2 … activos` (nunca llegaba a `antos>`) en cualquier configuración con varias funciones PCI —el caso típico de UTM: red, audio, xHCI, blk, rng—. El driver VirtIO-Input no reconocía la interrupción del transporte MMIO (`InterruptACK`), así que la línea se re-asertaba sin fin. Corregido: `poll_events()` hace `InterruptACK`; además el despachador de IRQ enmascara cualquier INTID sin manejador que se repita en bucle (`irq-storm … enmascarada`), y `enable_peripheral_irqs()` deja de habilitar en bloque las líneas PCIe INTx (el kernel sirve PCIe por sondeo).
- **UTM (macOS Apple Silicon):** ejecución verificada por **arranque directo `-kernel` + `ramfb`** (VM *Emulate → Linux → boot from kernel image*, `UEFI Boot` OFF, `virtio-keyboard-device`/`virtio-tablet-device`): llega a `antos>` con ventana gráfica y entrada, incluso con el juego completo de dispositivos PCI de UTM.
- **Puntero VirtIO-Input:** el cursor recorría ~1/32 del movimiento real del ratón (doble escalado: `EvdevAccumulator::flush()` ya pasaba a píxeles y `Cursor::move_abs()` volvía a dividir por 32768). `flush()` emite ahora el rango canónico `0..=32767` y el escalado a píxeles lo hace sólo el cursor.
